...
void read_data(FILE* in, FILE* out, FILE* data, FIDObj* array, StringHashTable* table, uint32_t numEle){
	...
	while (row_length > 0){
		...
		if (row_attr[0] == ';'){
			...
			char* set = (char*)malloc(17*sizeof(char));

			sscanf(row_attr, "%s", set);
			if (strcmp(set, "exists") == 0){
				char* name = (char*)malloc(124*sizeof(char));
			...
			//Calculates the details of section and lists the description
			else if (strcmp(set, "details_of") == 0){
				char* name = (char*)malloc(124*sizeof(char));
                		char* state = (char*)malloc(3*sizeof(char));
				sscanf(row_attr, "%*s\t%[^\t]%s", name, state);
				strcat(name, state);
			...
			//Calculates the distance between two points or latitudes and longitudes.
			else if (strcmp(set, "distance_between") == 0){
				int id1;
				int id2;
